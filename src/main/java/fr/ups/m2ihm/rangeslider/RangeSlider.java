...
public class RangeSlider extends JLayeredPane {
        ...
        public void mouseDragged(MouseEvent e) {
            switch (state) {
                ...
                case Dragging:
                    ...
                    x0 = e.getX();
                    model.setRangeValues(model.getMinValue() + mapToValue(e.getX() - x0),
    ...
    public RangeSlider() {
        ...
        model.addPropertyChangeListener(RangeSliderModel.PROP_MINVALUE, (e) -> {
            ...
            paintMinThumb();
            //Configure range
            range.setMinValue((double) e.getNewValue());
            paintRange();
        });
        ...
        model.addPropertyChangeListener(RangeSliderModel.PROP_MAXVALUE, (e) -> {
            ...
            paintMaxThumb();
            //Configure range
            range.setMaxValue((double) e.getNewValue());
            paintRange();
        });
    ...
    private void paintMinThumb() {
        int minThumbRelativeX = mapToPosition(minThumb.getCurrentValue());
    ...
    private void paintMaxThumb() {
        int maxThumbRelativeX = mapToPosition(maxThumb.getCurrentValue());
        maxThumb.setBounds(maxThumbRelativeX - groupHeight, groupY - groupHeight, groupHeight * 2, groupHeight * 2);
    }

    private void paintRange() {
        int rangeStart = mapToPosition(range.getMinValue());
        int rangeEnd = mapToPosition(range.getMaxValue());
